// Package http provides an HTTP client for the Canadian service.
package http
...
func New(instance string, options ...ClientOption) (pb.CanadianServer, error) {
	var cc clientConfig
	...
	var TranslateZeroEndpoint endpoint.Endpoint
	{
		TranslateZeroEndpoint = httptransport.NewClient(
			"get",
			copyURL(u, "/canadian"),
			svc.EncodeHTTPTranslateZeroRequest,
			svc.DecodeHTTPTranslateResponse,
			clientOptions...,
		).Endpoint()
	}

	return svc.Endpoints{
		TranslateEndpoint: TranslateZeroEndpoint,
	}, nil
}
